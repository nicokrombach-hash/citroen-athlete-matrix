...
function compressImage(file: File): Promise<string> {
  return new Promise(resolve => {
    ...
    reader.onload = e => {
      const img = new Image()
      img.onload = () => {
        const SIZE=220, ratio=Math.min(SIZE/img.width,SIZE/img.height,1)
...
function EditView({ data, isNew, onSave, onDelete, onBack }: {
  ...
  const handleImage = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    setForm(f=>({...f,image:b64})); e.target.value=''
  }
    ...
    <div style={{maxWidth:680,margin:'0 auto',padding:'20px 24px'}}>
      ...
      <div style={{display:'flex',alignItems:'center',gap:16,marginBottom:20,padding:'14px 16px',background:'#f5f5f5',borderRadius:8,border:'0.5px solid #e2e2e2'}}>
        ...
        <div style={{display:'flex',flexDirection:'column',gap:8}}>
          ...
          {form.image && <button onClick={()=>setForm(f=>({...f,image:null}))} style={{background:'none',border:'none',color:'#aaa',fontSize:12,cursor:'pointer',padding:0,textAlign:'left',fontFamily:'inherit'}}>✕ Bild entfernen</button>}
        </div>
      </div>
      ...
      <div style={{display:'grid',gridTemplateColumns:'1fr 1fr',gap:12,marginBottom:24,padding:'14px 16px',background:'#f5f5f5',borderRadius:8,border:'0.5px solid #e2e2e2'}}>
        <div>
          ...
          <div style={{display:'flex',alignItems:'center',gap:6,marginTop:7}}>
            ...
            <div style={{flex:1,height:4,background:'#e2e2e2',borderRadius:2,overflow:'hidden'}}><div style={{width:`${costScore/10*100}%`,height:'100%',background:'#B8860B',borderRadius:2}}/></div>
            <span style={{fontSize:11,fontWeight:500,color:'#B8860B'}}>{costScore.toFixed(1)}</span>
          </div>
        </div>
        <div>
          ...
          <div style={{display:'flex',alignItems:'center',gap:6,marginTop:7}}>
            ...
            <div style={{flex:1,height:4,background:'#e2e2e2',borderRadius:2,overflow:'hidden'}}><div style={{width:`${reachScore/10*100}%`,height:'100%',background:'#0F7E45',borderRadius:2}}/></div>
            <span style={{fontSize:11,fontWeight:500,color:'#0F7E45'}}>{reachScore.toFixed(1)}</span>
          </div>
        </div>
      </div>
      ...
      <div style={{display:'flex',gap:8,paddingTop:16,borderTop:'0.5px solid #e2e2e2',marginTop:8}}>
        {!isNew && <button onClick={onDelete} style={{background:'none',border:'0.5px solid #e2e2e2',color:'#aaa',padding:'8px 14px',borderRadius:6,fontSize:13,cursor:'pointer',fontFamily:'inherit'}}>Löschen</button>}
...
export default function Home() {
  ...
  const openAdd = () => { setEditData({ id: Date.now(), name:'', sport:'', image:null, cost:'', reach_sm:'', scores:blankScores() }); setView('edit') }
  ...
  const handleDelete = async () => {
    if (!editData || !confirm('Athleten wirklich löschen?')) return
    try {
