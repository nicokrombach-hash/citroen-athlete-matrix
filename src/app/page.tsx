...
function EditView({ data, isNew, onSave, onDelete, onBack }: {
  ...
  const handleImage = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    else alert('Upload fehlgeschlagen. Prüfe ob der Bucket "athlete-images" existiert und public ist.')
    setUploading(false)
    e.target.value=''
  }
            ...
            {BY_COMP[k].map((cc,i)=>{
              ...
              return (
                <div key={i} style={{display:'flex',alignItems:'center',gap:8,marginBottom:7,padding:'5px 8px',background:'#f5f5f5',borderRadius:4}}>
                  ...
                  <div style={{width:60,height:3,background:'#e2e2e2',borderRadius:2,overflow:'hidden'}}><div style={{width:`${(sc/10*100).toFixed(0)}%`,height:'100%',background:isM?c.color:'#c0c0c0'}}/></div>
  ...
  return (
    <div style={{minHeight:'100vh',background:'#fff'}}>
      <header style={{display:'flex',alignItems:'center',gap:12,padding:'0 24px',height:58,borderBottom:`2.5px solid ${RED}`,background:'#fff',position:'sticky',top:0,zIndex:10}}>
        ...
        {view==='grid'&&<button onClick={openAdd} style={{marginLeft:'auto',display:'flex',alignItems:'center',gap:6,background:RED,color:'#fff',border:'none',padding:'8px 16px',borderRadius:6,fontSize:12,fontWeight:500,cursor:'pointer',fontFamily:'inherit'}}>+ Athlet hinzufügen</button>}
      </header>
      {view==='grid'?(
