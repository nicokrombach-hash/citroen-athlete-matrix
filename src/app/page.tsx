    ...
    <div style={{ maxWidth: 680, margin: '0 auto', padding: '20px 24px' }}>
      ...
      <div style={{ marginBottom: 20 }}>
        ...
        <div style={{ display: 'flex', gap: 8, alignItems: 'center' }}>
          ...
          {form.image && (
            <button onClick={() => setForm(f => ({ ...f, image: null }))} style={{ background: 'none', border: 'none', color: '#aaa', fontSize: 12, cursor: 'pointer', padding: 0, fontFamily: 'inherit' }}>
              Bild entfernen
            </button>
          )}
        </div>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 12, marginBottom: 16 }}>
        <div>
          ...
          <input type="text" value={form.name} onChange={e => updateField('name', e.target.value)} placeholder="Vor- und Nachname" style={{ width: '100%', padding: '8px 10px', border: '1px solid #e2e2e2', borderRadius: 6, fontSize: 13, fontFamily: 'inherit', outline: 'none' }} />
        </div>
        <div>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 12, marginBottom: 24, padding: '14px 16px', background: '#f5f5f5', borderRadius: 8, border: '0.5px solid #e2e2e2' }}>
        ...
        <div>
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: 6, marginTop: 7 }}>
            ...
            <div style={{ flex: 1, height: 4, background: '#e2e2e2', borderRadius: 2, overflow: 'hidden' }}>
              <div style={{ width: `${computeReachScore(form.reach_sm) / 10 * 100}%`, height: '100%', background: '#0F7E45', borderRadius: 2 }} />
            </div>
      ...
      {Object.entries(CATS).map(([k, c]) => {
        ...
        return (
          <div key={k} style={{ marginBottom: 20 }}>
              ...
              <div key={cr.key} style={{ marginBottom: 10 }}>
                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 3 }}>
                  <label style={{ fontSize: 12, color: '#1a1a1a' }} title={cr.hint}>
                    {cr.label} <span style={{ fontSize: 10, color: '#aaa' }}>x{cr.w}</span>
                  </label>
            ...
            <div style={{ fontSize: 10, color: '#aaa', textTransform: 'uppercase', letterSpacing: '0.08em', margin: '10px 0 6px', paddingTop: 8, borderTop: '0.5px dashed #e2e2e2' }}>
              Auto-berechnet
            </div>
            {BY_COMP[k].map((cc, i) => {
